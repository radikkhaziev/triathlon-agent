...
async def process_activity_hrv(activity: ActivityRow) -> str:
    ...
    try:
        ...
        if result["thresholds"]:
            ...
            row.hrvt2_hr = thresholds.get("hrvt2_hr")
            row.threshold_r_squared = thresholds.get("r_squared")
        ...
        logger.exception("Error processing HRV for activity %s", activity_id)
        try:
            row = ActivityHrvRow(
                activity_id=activity_id,
                activity_type=activity_type,
...
# ---------------------------------------------------------------------------
# 8. Batch processing job
# ---------------------------------------------------------------------------


async def process_fit_job(batch_size: int = 2) -> list[tuple[str, str]]:
    """Process FIT files for unanalyzed bike/run activities.

    Runs every 5 min. Processes up to batch_size activities per run
    ...
    for i, activity in enumerate(activities):
        if i > 0:
            await asyncio.sleep(2)  # rate-limit: 2s between FIT downloads
        status = await process_activity_hrv(activity)
        logger.info("Activity %s: %s", activity.id, status)
        results.append((activity.id, status))
