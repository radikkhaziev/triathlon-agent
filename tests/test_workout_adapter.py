...
from data.models import RecoveryScore, WorkoutStep
...
class TestZoneEstimation:
    def test_power_z2(self):
        step = WorkoutStep(power={"low": 116, "high": 151, "units": "watts"})
        assert estimate_step_zone(step, ftp=233) == 2  # ~57% FTP

    def test_power_z3(self):
        step = WorkoutStep(power={"low": 180, "high": 210, "units": "watts"})
        assert estimate_step_zone(step, ftp=233) == 3  # ~84% FTP

    def test_power_z4(self):
        step = WorkoutStep(power={"low": 210, "high": 245, "units": "watts"})
        assert estimate_step_zone(step, ftp=233) == 4  # ~98% FTP

    def test_hr_z1(self):
        step = WorkoutStep(hr={"low": 90, "high": 110, "units": "bpm"})
        assert estimate_step_zone(step, lthr=153) == 1  # ~65% LTHR

    def test_hr_z4(self):
        step = WorkoutStep(hr={"low": 127, "high": 148, "units": "bpm"})
...
class TestComputeConstraints:
    def _recovery(self, score, category):
        return RecoveryScore(score=score, category=category, recommendation="")
...
class TestNeedsAdaptation:
    def test_z3_workout_needs_adaptation_at_z2_cap(self):
        steps = [
            WorkoutStep(text="Interval", duration=600, power={"low": 200, "high": 220, "units": "watts"}),
        ]
        assert needs_adaptation(steps, max_zone=2, ftp=233)

    def test_z2_workout_ok_at_z2_cap(self):
        steps = [
            WorkoutStep(text="Easy", duration=1800, power={"low": 116, "high": 151, "units": "watts"}),
        ]
...
class TestClampStep:
    ...
    def test_clamp_repeat_group(self):
        step = WorkoutStep(
            ...
            reps=3,
            steps=[
                WorkoutStep(text="Work", duration=300, power={"low": 200, "high": 240, "units": "watts"}),
                WorkoutStep(text="Rest", duration=120, power={"low": 95, "high": 119, "units": "watts"}),
            ],
        )
        clamped = clamp_step(step, max_zone=2, duration_factor=0.85, ftp=233)
...
class TestAdaptWorkout:
    ...
    def test_no_adaptation_when_excellent(self):
        recovery = RecoveryScore(score=90, category="excellent", recommendation="zone2_ok")
        result = adapt_workout(self._make_workout(), recovery, "green", tsb=5)
        assert result is None

    def test_adapts_when_moderate(self):
        recovery = RecoveryScore(score=55, category="moderate", recommendation="zone1_short")
    ...
    def test_adapted_has_correct_sport(self):
        recovery = RecoveryScore(score=45, category="moderate", recommendation="zone1_short")
    ...
    def test_adapted_steps_are_clamped(self):
        recovery = RecoveryScore(score=35, category="low", recommendation="skip")
    ...
    def test_run_adaptation(self):
        ...
        recovery = RecoveryScore(score=50, category="moderate", recommendation="zone1_short")
