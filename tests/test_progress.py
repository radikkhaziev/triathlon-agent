...
class TestIsZ2:
    ...
    def test_bike_z2_in_range(self):
        ...
        assert _is_z2(115, "bike") is True

    def test_bike_below_z2(self):
        assert _is_z2(95, "bike") is False

    def test_bike_above_z2(self):
        assert _is_z2(140, "bike") is False

    def test_run_z2_in_range(self):
        """Run Z2: 72-82% of LTHR=153 → 110-125 bpm."""
        assert _is_z2(118, "run") is True

    def test_run_below_z2(self):
        assert _is_z2(100, "run") is False

    def test_run_above_z2(self):
        assert _is_z2(135, "run") is False

    def test_swim_always_passes(self):
        """Swim has no HR filter."""
        assert _is_z2(150, "swim") is True
        assert _is_z2(80, "swim") is True

    def test_none_hr(self):
        assert _is_z2(None, "bike") is False
        assert _is_z2(None, "run") is False
