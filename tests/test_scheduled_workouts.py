...
- save_scheduled_workouts() sets last_synced_at
- get_scheduled_workouts_range() returns workouts + max last_synced_at
...
from data.database import get_scheduled_workouts_for_date, get_scheduled_workouts_range, save_scheduled_workouts
...
class TestSaveScheduledWorkoutsLastSyncedAt:
    async def test_sets_last_synced_at_on_insert(self):
        ...
        await save_scheduled_workouts([_make_workout(id=2001)])
        after = datetime.now(timezone.utc)

        rows = await get_scheduled_workouts_for_date(date(2026, 3, 25))
        assert len(rows) == 1
    ...
    async def test_updates_last_synced_at_on_upsert(self):
        await save_scheduled_workouts([_make_workout(id=2002, name="Old name")])
        rows = await get_scheduled_workouts_for_date(date(2026, 3, 25))
        first_sync = rows[0].last_synced_at

        # Upsert same ID with new name
        await save_scheduled_workouts([_make_workout(id=2002, name="New name")])
        rows = await get_scheduled_workouts_for_date(date(2026, 3, 25))
    ...
    async def test_all_rows_get_last_synced_at(self):
        ...
        await save_scheduled_workouts(workouts)

        for dt_offset in range(3):
            dt = date(2026, 3, 25) + timedelta(days=dt_offset)
            rows = await get_scheduled_workouts_for_date(dt)
            for row in rows:
...
class TestGetScheduledWorkoutsRange:
    async def test_returns_workouts_in_range(self):
        ...
        rows, _ = await get_scheduled_workouts_range(date(2026, 3, 23), date(2026, 3, 29))
        ids = {r.id for r in rows}
        assert ids == {3001, 3002, 3003}
        assert 3004 not in ids

    async def test_returns_last_synced_at(self):
        await save_scheduled_workouts([_make_workout(id=3010)])

        _, last_synced = await get_scheduled_workouts_range(date(2026, 3, 20), date(2026, 3, 30))
        assert last_synced is not None

    async def test_empty_range_returns_none_sync(self):
        rows, last_synced = await get_scheduled_workouts_range(date(2099, 1, 1), date(2099, 1, 7))
        assert rows == []
...
class TestScheduledWorkoutsEndpoint:
    ...
    async def test_workouts_appear_on_correct_day(self, client):
        ...
        await save_scheduled_workouts([_make_workout(id=5001, dt=date(2026, 3, 25))])
    ...
    async def test_duration_formatted(self, client):
        await save_scheduled_workouts([_make_workout(id=5010, moving_time=5400)])
    ...
    async def test_last_synced_at_in_response(self, client):
        await save_scheduled_workouts([_make_workout(id=5020)])
    ...
    async def test_includes_description(self, client):
        desc = "Warmup 10min\n3x10min @FTP\nCooldown"
        await save_scheduled_workouts([_make_workout(id=5030, description=desc)])
...
class TestSyncWorkoutsEndpoint:
    ...
    async def test_runs_sync_job(self, client):
        ...
        with (
            ...
            await save_scheduled_workouts([_make_workout(id=6001)])
