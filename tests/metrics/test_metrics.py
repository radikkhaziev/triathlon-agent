...
from data.intervals.dto import RhrStatusDTO, RmssdStatusDTO, TrendResultDTO
...
class TestCalculateTrend:
    def test_insufficient_data(self):
        t = calculate_trend([1.0, 2.0])
        assert t.direction == "stable"
        assert t.slope == 0.0

    def test_rising_trend(self):
        t = calculate_trend([10, 12, 14, 16, 18, 20, 22])
        assert t.direction in ("rising", "rising_fast")
        assert t.slope > 0
        assert t.r_squared > 0.9

    def test_declining_trend(self):
        t = calculate_trend([22, 20, 18, 16, 14, 12, 10])
        assert t.direction in ("declining", "declining_fast")
        assert t.slope < 0

    def test_stable_trend(self):
        t = calculate_trend([50, 50.1, 49.9, 50, 50.1, 49.9, 50])
        assert t.direction == "stable"

    def test_window_parameter(self):
        data = [10, 20, 30, 40, 50, 50, 50, 50, 50]
        t = calculate_trend(data, window=4)
        assert t.direction == "stable"

    def test_r_squared_range(self):
        t = calculate_trend([10, 20, 15, 25, 12, 22, 18])
...
class TestRmssdFlattEsco:
    ...
    def test_60d_baseline_with_long_history(self):
        result = rmssd_flatt_esco(self._history(70))
...
class TestCalculateEss:
    def test_threshold_effort_one_hour(self):
        ess = calculate_ess(60, 153, 42, 182)
...
class TestCalculateDailyEss:
    ...
    def test_multiple_activities_sum_greater_than_single(self):
        ...
        single = calculate_daily_ess([_activity(3600, 140)], hr_rest=42, hr_max=182, lthr=158)
    ...
    def test_no_hr_returns_zero(self):
        ...
        result = calculate_daily_ess([_activity(3600, None)], hr_rest=42, hr_max=182, lthr=158)
        assert result == 0.0

    def test_zero_hr_returns_zero(self):
        """Activity with average_hr=0 is skipped (below resting HR guard)."""
        result = calculate_daily_ess([_activity(3600, 0)], hr_rest=42, hr_max=182, lthr=158)
        assert result == 0.0
    ...
    def test_mixed_valid_and_no_hr_ignores_invalid(self):
        ...
        valid_only = calculate_daily_ess([_activity(3600, 140)], hr_rest=42, hr_max=182, lthr=158)
...
class TestCalculateBanisterForDate:
    ...
    def test_ess_today_matches_target_date_activities(self):
        ...
        expected_ess = calculate_daily_ess(target_activities, self._HR_REST, self._HR_MAX, lthr=self._LTHR)
...
class TestCombinedRecoveryScore:
    def _rmssd(self, status="green", cv=5.0) -> RmssdStatusDTO:
        return RmssdStatusDTO(
            ...
            upper_bound=55,
            cv_7d=cv,
            trend=TrendResultDTO(direction="stable", slope=0, r_squared=0.5, emoji="→"),
        )

    def _rhr(self, status="yellow") -> RhrStatusDTO:
        return RhrStatusDTO(status=status, days_available=30, days_needed=0, rhr_today=42, rhr_30d=42)
    ...
    def test_no_sleep_renormalises_weights(self):
        """When sleep_score is None, weights are renormalised (no sleep component)."""
        with_sleep = combined_recovery_score(
            self._rmssd(),
            self._rhr(),
            banister_recovery=70,
            sleep_score=70,
        )
        without_sleep = combined_recovery_score(
            self._rmssd(),
            self._rhr(),
            banister_recovery=70,
            sleep_score=None,
        )
        assert without_sleep.components["sleep"] is None
        assert with_sleep.components["sleep"] is not None
