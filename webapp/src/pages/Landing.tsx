...
// Ported 1:1 from the Claude Design handoff (landing/landing-variant-a.jsx +
// landing-shared.jsx + landing-app.jsx, final revision q3xdSpTQ...).
// Rendered with inline styles to stay pixel-faithful to the prototype; the
// BRAND tokens mirror the Halo CSS vars. Shown only to unauthenticated
// visitors at `/` (see App.tsx). Language follows the app-wide i18n locale.
import { useEffect, useState } from 'react'
import type { CSSProperties, ReactNode } from 'react'
import { Link } from 'react-router-dom'
import { useTranslation } from 'react-i18next'
import { BRAND, COPY } from '../components/landing/landingContent'
import type { Lang, LandingCopy } from '../components/landing/landingContent'
import { PhoneFrame, AppScreen } from '../components/landing/PhoneMockups'

const TG_URL = 'https://t.me/endurai_bot'
const GITHUB_URL = 'https://github.com/radikkhaziev/triathlon-agent'
const DEMO_PATH = '/login'
...
function VariantA({ t, lang, wide, onToggleLang }: VariantProps) {
    ...
    <div style={{ width: '100%', maxWidth: wide ? 1180 : 460, margin: '0 auto', minHeight: '100%',
      ...
      <div style={{ ...sec(), display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: wide ? 26 : 10 }}>
          ...
          <button onClick={onToggleLang} aria-label={lang === 'ru' ? 'Switch to English' : 'Переключить на русский'}
        ...
        <div style={wide
          ...
          <div style={{ textAlign: wide ? 'left' : 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: wide ? 'row' : 'column', gap: 12, marginTop: 28,
              ...
              <Link to={DEMO_PATH} style={{ ...aBtnPrimary, width: wide ? 'auto' : '100%' }}>{t.cta_demo} →</Link>
          ...
          <div style={wide ? { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 56, alignItems: 'center' } : {}}>
            <div>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: '20px 0 0', display: 'grid', gap: 12 }}>
                {t.tg_bullets.map((b, i) => (
                  <li key={i} style={{ display: 'flex', gap: 11, fontSize: 14.5, lineHeight: 1.5, color: BRAND.ink }}>
      ...
      <div id="demo" style={{ ...sec(), paddingTop: wide ? 80 : 56, paddingBottom: wide ? 80 : 56, textAlign: 'center' }}>
        <div style={{ background: BRAND.ink, color: '#fff', borderRadius: 28, padding: wide ? '64px 26px' : '44px 26px',
          ...
          <Link to={DEMO_PATH} style={{ ...aBtnPrimary, background: BRAND.cobalt, marginTop: 26, position: 'relative', boxShadow: '0 14px 30px -10px rgba(59,109,255,0.6)' }}>{t.cta_demo} →</Link>
        </div>
      </div>
...
// ── page shell — language follows the app-wide i18n locale ───────────────
...
export default function Landing(): ReactNode {
  ...
  const t = COPY[lang]
  const toggleLang = () => { i18n.changeLanguage(lang === 'ru' ? 'en' : 'ru') }

  return (
    <div style={{ minHeight: '100%', background: BRAND.warm }}>
      <VariantA t={t} lang={lang} wide={wide} onToggleLang={toggleLang} />
    </div>
  )
}
