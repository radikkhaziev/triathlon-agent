...
## Claude model migration — Sonnet 4.6 → Sonnet 5 (2026-09-22)

Все 6 call-site'ов переведены с `claude-sonnet-4-6` (legacy с 2026-09) на `claude-sonnet-5`: чат (`bot/agent.py`), утренний + недельный отчёты (`tasks/tools.py`, `WEEKLY_MODEL`), подпись активности (`tasks/actors/activities.py`), weekly changelog (`tasks/actors/changelog.py`), race plan (`data/race_plan_service.py`). Прайс $2/$10 за MTok — с сентября 2026 стандартный (повышение до $3/$15 отменено); токенизатор ~+30% токенов на тот же текст.

- **Ломающие изменения Sonnet 5, закрытые в коде:** non-default `temperature` → 400 (убран из changelog вместе с константой); `thinking` без параметра = adaptive (на 4.6 — выключен), thinking-токены входят в `max_tokens` → на неагентных маршрутах (подпись активности 220 tok, changelog 800 tok, race plan с forced `tool_choice`) `thinking={"type": "disabled"}` явно; на tool-loop маршрутах adaptive оставлен, `max_tokens` удвоены (чат 2048→4096, дефолт цикла и отчёты 4096→8192, race plan 2048→4096).
- **Формат ответа:** с thinking первый блок `content[0]` может быть не `text` — activities/changelog берут первый `text`-блок; tool-loop'ы уже возвращали весь `response.content` в assistant-turn (thinking-блоки сохраняются) и фильтровали по `type == "text"`.
- **Провенанс:** `RACE_PLAN_MODEL_VERSION` → `v2-2026-09-22` (правило `docs/RACE_PLAN_SPEC.md` §11).
- **Проверено live** нашим ключом: forced `tool_choice` на Sonnet 5 через Claude API работает и не запускает thinking (`thinking_tokens=0`), форма с `thinking: disabled` принимается; SDK `anthropic 0.96.0` пропускает параметры как есть.
- **Tests:** `tests/tasks/test_activity_signature.py::TestActorRenameActivityClaudePath` (thinking→text extraction + fallback без text-блока), `tests/tasks/test_weekly_changelog.py::TestCallClaude` (форма запроса), `tests/ai/test_chat.py` (chat 4096 vs default 8192).
- **Follow-ups:** стоимость чата/отчётов теперь включает thinking при `effort: high` по умолчанию — замерить `scripts/measure_report_cost.py` до тюнинга `effort`; `docs/MODEL_BILLING_SPEC.md` §2.2/§2.3 пересчитаны под $2/$10, тир `sonnet46` под вопросом. Строки `claude-sonnet-4-6` в `tests/db/test_weekly_report.py` / `tests/api/test_weekly_reports_routes.py` — фикстурные значения колонки `weekly_reports.model`, не модель.
...
- **Phase 3 — бюджетный резерв бэкфилла.** `actor_bootstrap_step` паузит чанк до 00:00 UTC (+jitter), если `remaining_day < BOOTSTRAP_DAILY_RESERVE (1500) + 3 × активностей`; проверка до `save_bulk`. Сентинел `QUOTA_PAUSED:<iso>` в `last_error` (`UserBackfillState.mark_quota_paused`, чистится `advance_cursor`), watchdog пропускает паузу до `resume + 15 мин`, API схлопывает в `QUOTA_PAUSED`, webapp показывает нейтральное «пауза по лимиту API» (сентинел общий для 15-минутного окна и дневной квоты). Ревью: `advance_cursor(expected_cursor=)` стал CAS + дроп «superseded generation» на входе (спящая копия + кик watchdog'а / `--force` форкали цепочку), guard от вечной паузы при `reserve + est_cost > limit_day`, единый штамп паузы на весь `_process_chunk`. Тесты: `TestQuotaReserve` в `test_bootstrap_actors.py`, `TestWatchdogQuotaPause` / `TestParseQuotaPause` в `test_scheduler_watchdog.py`, `tests/db/test_backfill_state.py`, `TestSecondsUntilQuotaReset` в `test_intervals_client.py`.
- **Pending:** Phase 4 очередь `backfill` (условно), письмо в Intervals про лимит.
