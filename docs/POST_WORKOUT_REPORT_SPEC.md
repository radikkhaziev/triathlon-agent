...
Если cost-gate проходит → переходим к **signal-gate (deterministic, ДО Claude-вызова)**. Без явного сигнала разбор **не генерируется**: storage + API tokens + alert-fatigue не стоят filler-комментариев типа «Z2 60 мин, ничего особенного». Webapp при `ai_recommendation IS NULL` рендерит плейсхолдер «без AI-комментария» (см. §8). По метрикам Phase 1 ожидаем ~30-50% сессий без AI-блока — это **by design**, а не баг.
...
def evaluate_signal_gate(activity_id: str, user: UserDTO) -> SignalResult:
    ...
    # (see §3 + data/metrics.py:is_valid_for_decoupling: VI ≤ 1.10, ≥70% Z1+Z2,
    # bike ≥ 60min, run ≥ 45min, swim excluded). На коротком интервальном ride
    # decoupling 15% — артефакт HR-задержки на rest/work циклах, не сигнал.
    # Тот же фильтр на history-кандидатов: невалидных исключаем из медианы,
    # иначе она сама смещается шумом коротких ride'ов. Empirically выявлено
    # в demo 2026-05-14 (i148059070 — 36-мин threshold intervals: decoupling
    # 15.3% vs медиана 3.2% → false-positive push без фильтра).
    if (
        similar.matches
        ...
        if valid_history:
            ...
            if abs(detail.decoupling_pct - median) >= 1.5:
                return SignalResult(
...
**Если signal.passed == False** → actor завершает БЕЗ Claude-вызова. `ai_recommendation` остаётся `NULL`, webapp на `/activity/:id` рендерит «без AI-комментария» (см. §8). Никакого filler-разбора «Z2 60 мин, ничего особенного» в БД не лежит — пустота лучше воды.

**Если passed == True** → `signal.reason` + `signal.evidence` пробрасываются в user prompt как **anchor** (см. §5). Claude обязан остаться на этом сигнале и не растекаться по сессии. Push в TG attempted после persist'а и валидатора.
...
- `post_workout.gate.signal_skipped_total` — сколько сессий ушло в пустоту
- `post_workout.report.length_lines_histogram` — фактическое распределение длины (цель: median 2-3, max 5)

Через 2-3 недели после выкатки можно подтянуть пороги (decoupling Δ, RPE Δ, race window) на реальных данных.
...
def actor_compose_post_workout_report(user: UserDTO, activity_id: str):
    ...
    # «без AI-комментария» placeholder. This is the path for ~30-50% of sessions.
    try:
...
- [ ] **PW-7 — Signal-gate `_has_interesting_signal`.** Compliance/decoupling-outlier/drift/RPE-mismatch/PR/race-window. **Decoupling-outlier триггер обязан фильтровать через `is_valid_for_decoupling()` (`data/metrics.py`) и сегодняшнюю activity, и каждого history-кандидата** — иначе короткие интервалы дают false-positive push (см. §4 + demo 2026-05-14). Метрики `post_workout.gate.signal_passed_total{reason}` для тюнинга. **Persist всегда, push только при passed=true.**
