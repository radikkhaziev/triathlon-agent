...
| Repeat-scope overflow not caught by the bridge heuristic | Defensive log only — `humango_to_intervals_steps` parses HumanGo's workout-level «total distance: N meters» header, sums parsed swim distance recursively, and emits a `WARNING` log when parsed > announced × 1.2. Does **not** fail-closed: a slightly-wrong structured push is still better UX than a flat description that watches can't structure (athletes lose the per-step chart in Intervals UI + the FIT corridor on Garmin). Surfaces drift so a future SPEC iteration can add another heuristic. |
