...
2. **Steady-state Z2 only:** average HR inside the Z2 band from LTHR. The live system uses the athlete's own zones synced from Intervals.icu (`athlete_settings.hr_zones`); bike 68–83% / run 72–82% are fallback defaults only (see `docs/BUSINESS_RULES.md`).
