...
- **Dispatch plan:**
  - **Persist** через `ActivityAchievement.save_bulk(user_id, activity_id, activity)` — таблица `activity_achievements` (миграция `u1b2c3d4e5f6`). Одна строка на каждый item в `icu_achievements[]` + одна синтетическая `FTP_CHANGE`-строка если `icu_rolling_ftp_delta != 0` (с `extra={"delta": ...}`). Идемпотентно через `UNIQUE(user_id, activity_id, achievement_id)` — Intervals.icu может ретрансмитить webhook. Источник истины для будущего social-sharing UI (списки power PR'ов / FTP-апдейтов / milestones).
  - Telegram notification через `actor_send_achievement_notification` при наличии achievements (отправляется ПОСЛЕ persist, чтобы Telegram-outage не стоил данных).
  - Если `icu_rolling_ftp_delta != 0` → `actor_sync_athlete_settings(user)` для refresh пороговых значений (Phase 2 — пока FTP_CHANGE-строка только в `activity_achievements`, не в `athlete_settings`).
  - Persistence error не блокирует Telegram-нотификацию (`try/except` + `sentry_sdk.capture_exception`) — realtime ping важнее архива.
