Создам полную спеку — соберу всё из диалога в один документ.Bash недоступен в этой сессии. Создам файл напрямую через create_file.Файловая система и bash недоступны в этой сессии — создать файл напрямую не могу. Выведу полную спеку в чат, ты сможешь скопировать в `RAMP_TEST_SPEC.md` в репозитории `triathlon-agent`, рядом с `REMOTION_SPEC.md`.
