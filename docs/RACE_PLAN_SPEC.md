...
- **`model_version`** провенанс. Bump при изменении промпта / JSON schema / Claude модели (текущая `claude-opus`).
...
5. **Claude call** — `claude-opus`, forced `tool_use=submit_race_plan`, `max_tokens=2048`. `ApiUsageDaily.increment` после ответа (включая validator-reject — токены уже потрачены).
...
| 2026-05-09 | §12 | Bumped Claude model to opus (был sonnet-4-6) | Quality > tokens для once-per-race call |
