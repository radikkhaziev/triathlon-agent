...
**Текущее решение (`workout_cards.py:415-429`, корректно):**
...
- **Top-level `description`** — то же содержимое (`Exercises: N, ~M min\n<URL>`). Установка гейтнута через `_TOP_LEVEL_DESC_SPORTS = frozenset({"Run", "Ride", "Other"})` — Swim исключён из-за исторической parse-failure-induced steps-strip (см. §«Парсерные ловушки»). Intervals.icu принимает plain-text top-level description для `Other` без проблем (подтверждено probe 109776724 + 109777679, 2026-05-12) — URL виден в web/mobile UI.

**Историческая аномалия (`108298132`, 2026-05-11):** зарядка пушнута с `description: None` несмотря на наличие гейта в коде на тот момент. Скорее всего deploy-window: фикс `_TOP_LEVEL_DESC_SPORTS` был мержен в `32465557` (5 мая), но в prod дошёл позже — все зарядки 6-11 мая в этом окне. Fresh push 12 мая показал корректный shape. Backfill исторических зарядок возможен (через `update_event` по сохранённым `AiWorkout.intervals_id`), но low-value — они уже отзанимались.
