...
`CHAT_TOOLS` — копия `MORNING_TOOLS` из Phase 2 + chat-only tools. 16 инструментов: 14 из MORNING_TOOLS (7 основных + 7 опциональных) + 2 chat-only.

```python
# ai/tool_definitions.py
CHAT_TOOLS = [
    *MORNING_TOOLS,          # 14 tools для чтения данных
    SAVE_MOOD_CHECKIN_TOOL,   # chat-only: запись mood check-in
    GET_MOOD_CHECKINS_TOOL,   # чтение mood (есть и в MORNING_TOOLS, но для ясности)
]
```

### Chat-only tools (не в MORNING_TOOLS)

| Tool | Описание | Handler |
|---|---|---|
| `save_mood_checkin` | Записать эмоциональное состояние (energy/mood/anxiety/social 1-5 + note) | `data/database.py → save_mood_checkin()` |

Claude замечает эмоциональный контекст в сообщении → вызывает `save_mood_checkin` автономно. Примеры триггеров: "устал", "плохо спал", "на подъёме", "стресс на работе", упоминание тревоги, энергии, настроения.

> **Важно:** `save_mood_checkin` не включён в `MORNING_TOOLS` — утренний анализ только читает данные, не записывает.
...
| "Устал сегодня, ноги тяжёлые" | save_mood_checkin, get_recovery | Записал mood (energy=2, mood=3) + совет по recovery |
