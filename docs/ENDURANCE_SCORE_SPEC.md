...
Заменяет статический `EnduranceScoreCard` в `webapp/src/pages/DashboardLoadTab.tsx:84-99`. Структура карточки и детального экрана **уже спортирована в дизайне** (`direction-b-halo.jsx:3396-3622`) — нужно (a) заменить static fixture на fetched data, (b) обновить zone-конфигурацию: 6 → 5 зон, новые пороги и лейблы (см. §3.8), `ENDURANCE_MAX` 9500 → 8000.

**Карточка (`/dashboard?tab=load` → top):**
- Eyebrow «Endurance Score» (RU «Запас выносливости») + `i`-info-tooltip: «Композитный балл по всем видам спорта. Зоны: Растренирован → Восстанавливаюсь → Поддерживаю → Развиваюсь → На пике. «Развиваюсь» и «На пике» — целевые состояния для гоночной формы.»
- `EnduranceGauge` (SVG **5-segment** arc, marker на текущем score; обновить с 6 → 5 segments) — компонент **частично готов** в дизайне (`direction-b-halo.jsx:3444`), нужен apply 5-zone config (см. §3.8).
- Milestone-badge plate под gauge'м (см. §3.9) — опциональная, рендерится только если правило сработало.
- Per-sport breakdown — 2-column grid с цветными точками + %.
- Period-filter chips `1M / 3M / 6M / 1Y` — **Phase 2 only**; в Phase 1 chips скрыты, показывается фиксированный 12-week weekly trend (см. §9).
- Mini-chart с диапазоном `min — max` под filter'ом.
- «tap for trend ›» → push `/dashboard?tab=load&endurance=1` (или local state — TBD при имплементации).

**Детальный экран (`BEnduranceScoreDetail`):**
- Restate gauge (260px) + «Δ vs last week».
- Trend chart (line, hand-rolled SVG — паттерн уже принят в Load-табе). Phase 1 — 12-week weekly. Phase 2 — period-filtered (1m/3m/6m/1y) с daily resolution.
- Zone legend — **5 строк** с цветом, лейблом, диапазоном; current zone подсвечена.

i18n: namespace `load.endurance.*` (вынести существующий `load.endurance_desc` туда же).
...
python -m cli backfill-endurance-scores
