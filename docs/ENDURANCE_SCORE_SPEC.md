...
Карточка в Dashboard Load-табе (`webapp/src/pages/DashboardLoadTab.tsx`). Tap → full-screen detail `webapp/src/pages/EnduranceDetail.tsx` (route `/trends/endurance`) с трендом, current zone и легендой зон.

Бэкенд (as shipped — code-truth; пути ниже отражают где реально лежит, не первоначальный план):
1. **Pure module** `data/endurance_score.py` — формула + zones + badges. Сервис-слой `data/endurance_score_service.py` (`compute_for(user_id, ref_date)` → `EnduranceScoreResult`: overall + per-sport sub-scores + components).
2. **Storage** — таблица `endurance_scores`, daily-grain, UNIQUE `(user_id, snapshot_date)` — идемпотентный upsert. ORM: `data/db/endurance_score.py`; миграция `f6e360a8f4fa`.
3. **Triggers** (см. §7.0) — Level 1: после wellness/activities sync; Level 2: daily safety-net cron 18:30 Belgrade; Level 3 (read-only, deferred): morning report читает latest.
4. **API** — `GET /api/endurance-score?period=3m` (живёт в `api/routers/dashboard.py`, не в отдельном файле) → current + components + period-filtered time-series (1m/3m/6m/1y).
5. **Backfill CLI** — `python -m cli backfill-endurance-scores` (default: all active × 365 days), `cli.py:_backfill_endurance_scores`.
...
### 4.1 Таблица `endurance_scores`

Schema: ORM `data/db/endurance_score.py`; миграция `f6e360a8f4fa`. Daily-grain, `score INTEGER`, `vo2max_composite NUMERIC(5,1)`, `components JSONB` (`{long_term, recent, duration, consistency, recovery, per_sport, detrain_factor, ctl_peak_26w, ...}`), UNIQUE `(user_id, snapshot_date)` (идемпотентный upsert), index на `(user_id, snapshot_date DESC)`, FK `users(id) ON DELETE CASCADE`.

**Зачем daily, а не weekly** (изменено 2026-05-25): дизайн поддерживает фильтры `1M / 3M / 6M / 1Y`. Weekly даёт всего 4 точки на 1M — chart как палки. Daily: 30 точек на 1M, плавная линия. Storage тривиален (~30 KB JSONB/юзер/год). Weekly view derive'ится downsample'ом (Sunday-snapshots).

**Зачем JSONB components:** drill-down («почему score упал»), post-hoc анализ без перерасчёта, per-sport sub-scores без отдельной таблицы.
...
`GET /api/endurance-score?period={1m|3m|6m|1y}` — implementing code in `api/routers/dashboard.py` (`endurance_score()`). Auth `require_viewer` (demo-friendly — score не sensitive).

Returns `current` (today's row: `score` / `zone` / `vo2max_composite` / `components` / `per_sport[].{name,pct,sub_score}` / `delta_vs_week_ago` / nullable `badge` {id,label,icon} / `computed_at`) + `trend` (daily `{date, score}` series) + `period`. Reads daily snapshots from `endurance_scores`; falls back to on-the-fly compute when today's row is missing (cron not fired / backfill incomplete).

**Period → window:** `1m`=30 / `3m`=90 (default, training-cycle scope) / `6m`=180 / `1y`=365 daily points. Frontend may downsample 1Y to weekly for clarity.

---

## 6. Frontend integration

Реализация: карточка `webapp/src/components/halo/EnduranceScore.tsx`, детальный экран `webapp/src/pages/EnduranceDetail.tsx` (route `/trends/endurance`; legacy `/wellness/endurance` → redirect, см. `App.tsx`). Карточка в `DashboardLoadTab.tsx` над sport-свитчером. Дизайн-источник: `design-package/endurai/direction-b-halo.jsx`. Размещение менялось дважды — см. §12.1 decisions log (Wellness home → Trends/Load).

Ключевые визуальные контракты (детали — в .tsx):
- 5-segment SVG gauge, `ENDURANCE_MAX = 8000`, marker на current score.
- Milestone-badge plate под gauge'м (см. §3.9) — только если правило сработало; иначе «Прогресс +Δ vs last week».
- Per-sport breakdown — цветные точки + %.
- Detail: period-filter (1M/3M/6M/1Y, default 3M, refetch на switch), trend chart с zone bands (current opacity 0.12, прочие 0.06), y-axis snap к границам зон (3000/4500/5500/6500/8000, k-формат), zone-coloured line runs (`buildRuns` из TSB-графика `LoadDetail.tsx`; dots при `N ≤ 40`), 5-строчная zone legend с подсветкой current.
- Карточка владеет своим fetch'ем (`?period=3m`, читает только `current`); detail — своим с period state.

i18n: namespace `load.endurance.*` (исторический label, не location — перенос не требуется).
...
**Level 1 — real-time во время дня:** `actor_user_wellness` (`tasks/actors/wellness.py`) и `actor_fetch_user_activities` (`tasks/actors/activities.py`) после write'а fire `actor_snapshot_endurance_scores.send(user_id=...)`. Оба уже вызываются из cron + webhook'ов (WELLNESS_UPDATED, ACTIVITY_UPLOADED) — ES наследует ту же частоту.
...
**Level 3 — morning report (READ-only, НЕ триггер; deferred к Phase 3):** report просто читает `EnduranceScore.get_latest()`. Он **downstream от** `actor_user_wellness` (см. CLAUDE.md «Multi-Tenant Data Flow») — к моменту запуска wellness-actor уже отстрелил `actor_snapshot_endurance_scores` параллельно, свежий snapshot в таблице. Самостоятельный recompute из report'а создал бы race с wellness-trigger'ом и double-compute.
...
`tasks/scheduler.py` — job `endurance_scores_daily`, **каждый день 18:30 Belgrade** (`misfire_grace_time=3600`, `coalesce=True`) → `actor_snapshot_endurance_scores_all_users`. После того как Intervals.icu закончил sync wellness/activities (последние syncs 17:00–18:00). misfire-grace покрывает restart/deploy в пределах часа.
...
`tasks/actors/endurance.py` — два actor'а:
- `actor_snapshot_endurance_scores(user_id)` — compute + upsert today для одного юзера (Level 1 hooks + sub-job из all-users). **Идемпотентен — безопасно fire'ить много раз в день** (upsert на `(user_id, snapshot_date)` → ON CONFLICT DO UPDATE с актуальными данными). Skip если `result.score is None`.
- `actor_snapshot_endurance_scores_all_users()` — safety-net cron, итерирует активных юзеров (фильтр `athlete_id.isnot(None)`) и fire'ит per-user actor. Ловит юзеров без Level-1 fire'а сегодня + естественный decay (сессии выпадают из 28d окна).

### 7.3 Backfill CLI

`python -m cli backfill-endurance-scores` — `cli.py:_backfill_endurance_scores`. Flags: `--user-id` (default: все активные — отличие от positional-required `backfill-races`/`bootstrap-sync`), `--days` (default 365), `--force` (перезаписать существующие), `--dry-run` (печатает scope без write'а).

Поведение: резолвит юзеров → итерирует `today-days+1 → today` по дням → per day если `--force` или row отсутствует → compute + upsert. **Идемпотентен по умолчанию** (`--force=False` → no-op для уже-посчитанных дат, заполняет только пропуски). Per-user try/except (ошибка одного не валит команду; Sentry tag `user_id`/`ref_date`; exit-code 1 если ≥1 error). Pure-compute <100ms/день → ~30 сек × user; sequential приемлем для one-shot backfill'а (parallel — premature).
