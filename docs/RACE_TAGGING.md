...
> **Статус: ✅ Реализовано (PR #134).** Исключение: Telegram-уведомление при автосоздании Race намеренно пропущено (#3.3, пункт 2).

### Этап 1: Data Layer ✅

1. ✅ Миграция: `is_race` + `sub_type` на `activities` (`6c5f6ae05c97`)
2. ✅ Миграция: таблица `races` (`6c5f6ae05c97`)
3. ✅ Миграция: `is_race` на `training_log` (`883ad6dc7748`), `race_id` FK (`q7f8a9b0c1d2`)
4. ✅ Миграция: `Race.updated_at` + `normalized_pace_sec_km` (`r8a9b0c1d2e3`)
5. ✅ `ActivityDTO.is_race` + `sub_type`
6. ✅ `Activity.save_bulk` синкает новые поля, **с merge-логикой**: локально отмеченные гонки (через `tag_race`) переживают ресинк из Intervals.icu (`is_race = OR`, `sub_type = COALESCE`)
7. ✅ `_spec_get_activities` включает `race,sub_type`
8. ✅ ORM `Race` модель: `get_by_activity`, `get_range`

### Этап 2: MCP Tools ✅

1. ✅ `get_races` — список с fitness context и метриками активности
2. ✅ `tag_race` — ручная маркировка + автозаполнение wellness snapshot
3. ✅ `update_race` — обновление деталей
4. ✅ `get_activities` возвращает `is_race` + `sub_type`
5. ✅ `get_training_log` возвращает `is_race` + вложенный `race` объект (bulk-fetch, defence-in-depth по `user_id`)

### Этап 3: Sync & Automation ✅ (частично)

1. ✅ Автосоздание Race при синке — `actor_fill_training_log` → `_ensure_race_record` (с wellness snapshot)
2. ⛔ Telegram notification при новой гонке — **намеренно пропущено** (избыточно: уже есть post-race уведомление)
3. ✅ `actor_fill_training_log` заполняет `is_race` + `race_id` в TrainingLog
4. ✅ CLI `backfill-races` (`cli.py`)
5. ✅ Промпты: race-day checklist в `SYSTEM_PROMPT_V2`, инструкция по `tag_race`/`get_races` в `SYSTEM_PROMPT_CHAT`, race-format в `tasks/formatter._build_post_race_message`

### Этап 4: Webapp ✅

1. ✅ Race badge 🏁 в Activities page
2. ✅ `RaceSection` компонент в Activity detail (`webapp/src/pages/Activity.tsx`): название, тип, финиш/цель, дистанция, темп, место, покрытие, погода, RPE, fitness snapshot, заметки
3. ✅ `is_race` + вложенный `race` объект в `/api/activity/{id}/details`

### Предупреждения для будущих изменений

- **`Activity.save_bulk` merge semantics**: `is_race` работает как sticky bit (once tagged, stays tagged). `sub_type` сохраняет локальное значение через `COALESCE`. Intervals.icu **не** является источником правды для race-тегов — изменение флага `race` через Intervals.icu UI после локального тега **не будет** подхвачено.
- **`training_log.race_id` backfill**: для исторических записей, созданных до появления `race_id`, нужен SQL-бэкфилл:
  ```sql
  UPDATE training_log tl
  SET race_id = r.id, is_race = true
  FROM races r
  WHERE r.user_id = tl.user_id AND r.activity_id = tl.actual_activity_id AND tl.race_id IS NULL;
  ```
