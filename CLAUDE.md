...
All core modules done. Multi-tenant Phase 1.3 + Intervals.icu OAuth Phase 2 + OAuth bootstrap backfill Phase 1+2 + Webhook data capture Phase 1+2 + User-memory facts Phase 1 + ATP Phase 3 personal-patterns prompt enrichment complete. HRV collapsed to single algorithm (Flatt/Esco) in #307 — AIEndurance retired. Ramp-test Run protocol switched to pace-driven (`%pace` units, `event.target=PACE` auto-set via `has_pace_steps`) with an 8-step `80→115%` ladder calibrated against pace at HRVT2; ramp-pipeline gained AiWorkout fallback for ramp detection (defense-in-depth against missed `CALENDAR_UPDATED` webhooks), latest-ramp drift detection gated by `|drift|>5%` AND `R²≥0.7` (the prior 3-sample-avg + bootstrap paths were collapsed in 2026-05-08). `actor_update_zones` pushes **HRVT2** (anaerobic threshold) into Intervals.icu's `lthr` field, **pace at HRVT2** into `threshold_pace`, and **pow at HRVT2** into `ftp` (Ride only — issue #313, 2026-05-08; the prior HRVT1→`lthr` mapping was mis-aligning every Intervals zone by ~13%). `get_zones` MCP tool reshape (issue #313): sport-tagged keys `power_zones_bike`/`power_zones_run`, `pace_zones_run`/`pace_zones_swim`, dual-unit zone objects (raw % + absolute watts/sec). Webhook dispatchers 8/10 implemented. **Pending:** MT Phase 2 (JWT upgrade), retire legacy `INTERVALS_API_KEY`, user-memory Phase 2 extractor.
