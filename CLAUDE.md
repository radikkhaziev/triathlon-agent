...
4. **Taper Planner** — ✅ Phase 1 (pure `build_taper_plan` + tests) + ✅ Phase 2 (`get_taper_plan` MCP tool) + ✅ Phase 4 (webapp: `GET /api/taper-plan` + stepped budget line / window tint / RACE flag on LoadDetail TSS chart + race-day TSB landing dot; shared `data/taper_service.py` resolution). Remaining: Phase 3/5 (race-plan `taper` block, morning-report line) — optional, decide after the tool lives in chat. See `docs/TAPER_PLANNER_SPEC.md`
